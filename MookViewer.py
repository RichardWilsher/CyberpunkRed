...
def loadmookarmour(id, newmook):
    headarm = mookheadarmourdb.find("", "mookid", str(id))
    harmourtype = armourdb.find("", "id", str(headarm[0][2]))
    bodyarm = mookbodyarmourdb.find("", "mookid", str(id))
    barmourtype = armourdb.find("", "id", str(bodyarm[0][2]))
    if headarm[0][3] == "y":
...
def loadmookstats(id, newmook):
    stats = mookstatdb.find("", "mookid", str(id))
    stattitles = statsdb.find()
    mookstats = {}
...
def loadmookweapons(id, newmook):
    # complexity 22/15
    weapontypes = weapondb.find()
    qualitytypes = weaponqualitydb.find()
    weapons = mookweapondb.find("", "mookid", str(id))
    weaponlist = []
...
def loadmookskills(id, newmook):
    mookskills = {}
    skilltitles = skilldb.find()
    skills = mookskilldb.find("", "mookid", str(id))
...
def loadmookequipment(id, newmook):
    # complexity 16/15
    equipmenttable = equipmentdb.find()
    equipment = mookequipmentdb.find("", "mookid", str(id))
    mookequipment = []
...
def loadmookcyberwear(id, newmook):
    # complexity 16/15
    cyberweartable = cyberweardb.find()
    cyberwear = mookcyberweardb.find("", "mookid", str(id))
    mookcyberwear = []
...
def get_index(*arg):
    if mooks[mookchoosen.current()].role:
...
def get_weapons(*arg):
    ...
    if tempweaponname != "":
        weaponprofile = weapondb.find("", "name", weaponname)[0]
...
#main program start
# loading databases
mookdb = orm.mysqlorm("cpr.mooks")
mooktypedb = orm.mysqlorm("cpr.mook_type")
typedb = orm.mysqlorm("cpr.type")
weapondb = orm.mysqlorm("cpr.weapons")
skilldb = orm.mysqlorm("cpr.skills")
equipmentdb = orm.mysqlorm("cpr.equipment")
cyberweardb = orm.mysqlorm("cpr.cyberwear")
armourdb = orm.mysqlorm("cpr.armour")
weaponqualitydb = orm.mysqlorm("cpr.weapon_quality")
mookstatdb = orm.mysqlorm("cpr.mook_stat")
mookheadarmourdb = orm.mysqlorm("cpr.mook_head_armour")
mookbodyarmourdb = orm.mysqlorm("cpr.mook_body_armour")
mookweapondb = orm.mysqlorm("cpr.mook_weapon")
mookskilldb = orm.mysqlorm("cpr.mook_skill")
mookequipmentdb = orm.mysqlorm("cpr.mook_equipment")
mookcyberweardb = orm.mysqlorm("cpr.mook_cyberwear")
